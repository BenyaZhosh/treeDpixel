...
float octave_noise(float3 input, int octaves, float persistence, float lacunarity)
{
    float noise = 0;
    float level = 0;
    ...
    {
        float strength = pow(persistence, octave);
        noise += cnoise(input * pow(lacunarity, octave)) * strength;
        level += strength;
    }
...
float octave_noise(float3 input, int octaves)
{
    return octave_noise(input, octaves, 0.5, 2);
}
